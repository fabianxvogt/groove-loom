...
export default function Home() {
  ...
  const startTransport = async () => {
    try {
      ...
      const startedAt = performance.now(); setIsSuspended(false); setIsPlaying(true); setMessage(`${eventSummary(scheduled)} · live clock armed`); addLog('success', `Playing ${scheduled.length} seeded events from ${project.chain.length}-bar chain.`);
      intervalRef.current = window.setInterval(() => setPlayhead(Math.floor(((performance.now() - startedAt) / 1000 / (60 / project.bpm / 4)) % (project.chain.length * STEP_COUNT))), 40);
